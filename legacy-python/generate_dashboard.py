...
<body>
  ...
  <div class="container">
    ...
    <div class="metrics-row">
      ...
      <div class="metric-card">
        ...
        <div class="value" id="contracts-value">{h_star:,}</div>
    ...
    <div class="hedge-summary" id="hedge-summary">
  ...
  <script>
    (function() {{
      ...
      var salary = parseFloat(params.get('salary'));
      if (!salary || isNaN(salary) || salary <= 0) return;

      // Match pipeline sizing: 6 months of salary, $1 payout per contract
      var nContracts = Math.round(salary * 6 / 12);
      var contractPrice = 0.30;
      var totalCost = Math.round(nContracts * contractPrice);

      var contractsEl = document.getElementById('contracts-value');
      if (contractsEl) contractsEl.textContent = nContracts.toLocaleString();

      var summary = document.getElementById('hedge-summary');
      if (summary) {{
        summary.innerHTML =
          '<strong>Bottom line:</strong> Buy <strong>' + nContracts.toLocaleString() + ' contracts</strong> ' +
          'that pay out if US unemployment exceeds 8.0%. You pay <strong>$' + totalCost.toLocaleString() + '</strong> upfront. ' +
          'If unemployment gets that high, you receive <strong>$' + nContracts.toLocaleString() + '</strong> — enough to cover about 6 months of pay. ' +
          'In our simulation, that happens in <strong>{hedge_payout_rate:.1f}%</strong> of scenarios.';
      }}
    }})();
  </script>
</body>
